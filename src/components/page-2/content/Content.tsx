import cloud from "../assets/visual-cloud.png";
import citrus from "../assets/stock-citrus.jpg";

const svg2 = (
  <svg
    className="absolute -bottom-10 right-5 w-full"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 1440 128"
    aria-hidden="true"
  >
    <path
      fill="#d3d23c"
      fill-rule="evenodd"
      d="M-1 128h1441V10.346c-15.931 1.765-32.565 2.85-46.786 7.017-18.994 5.56-30.708 22.76-53.462 20.307-19.23-2.074-30.91-11.889-51.297-12.6-19.53-.693-40.107-1.67-58.942-7.002-21.126-5.972-35.514-5.135-58.943-2.415-19.923 2.285-45.998 8.164-66.307 6.447-20.595-1.769-32.584-20.241-54.547-18.186-15.56 1.442-28.06 12.484-47.419 8.861-16.9-3.164-25.886-13.905-46.005-12.678-18.156 1.115-35.18 10.787-51.368 14.799-9.106 2.218-18.602 6.33-28.485 7.212-8.117.743-17.501-1.64-25.455-.301-18.515 3.093-27.66 7.68-46.94 4.012-6.875-1.304-12.86-1.899-18.517-1.899-5.67 0-11.642.595-18.53 1.9-19.26 3.667-28.404-.92-46.926-4.013-7.96-1.338-17.34 1.044-25.469.3-9.87-.88-19.387-4.993-28.478-7.211C649.943 10.884 632.9 1.212 614.775.097c-20.137-1.227-29.124 9.514-46.05 12.678-19.335 3.623-31.833-7.419-47.392-8.861-21.964-2.055-33.94 16.417-54.555 18.186-20.32 1.717-46.378-4.162-66.3-6.447-23.428-2.72-37.81-3.557-58.956 2.415-18.843 5.331-39.387 6.31-58.917 7.001-20.392.712-32.074 10.527-51.29 12.6-22.8 2.454-34.474-3.927-53.467-9.487-19.321-5.657-43.09-5.63-63.489-9.344-17.823-3.224-27.253.743-45.783 3.785-22.191 3.647-34.161-.626-53.75-6.193C8.706 14.68 3.616 13.91-1 13.76V128Z"
    ></path>
  </svg>
);
const svg = (
  <svg
    className="absolute w-full"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 1440 128"
    aria-hidden="true"
  >
    <path
      fill="#f0f1d1"
      fill-rule="evenodd"
      d="M-1 128h1441V10.346c-15.931 1.765-32.565 2.85-46.786 7.017-18.994 5.56-30.708 22.76-53.462 20.307-19.23-2.074-30.91-11.889-51.297-12.6-19.53-.693-40.107-1.67-58.942-7.002-21.126-5.972-35.514-5.135-58.943-2.415-19.923 2.285-45.998 8.164-66.307 6.447-20.595-1.769-32.584-20.241-54.547-18.186-15.56 1.442-28.06 12.484-47.419 8.861-16.9-3.164-25.886-13.905-46.005-12.678-18.156 1.115-35.18 10.787-51.368 14.799-9.106 2.218-18.602 6.33-28.485 7.212-8.117.743-17.501-1.64-25.455-.301-18.515 3.093-27.66 7.68-46.94 4.012-6.875-1.304-12.86-1.899-18.517-1.899-5.67 0-11.642.595-18.53 1.9-19.26 3.667-28.404-.92-46.926-4.013-7.96-1.338-17.34 1.044-25.469.3-9.87-.88-19.387-4.993-28.478-7.211C649.943 10.884 632.9 1.212 614.775.097c-20.137-1.227-29.124 9.514-46.05 12.678-19.335 3.623-31.833-7.419-47.392-8.861-21.964-2.055-33.94 16.417-54.555 18.186-20.32 1.717-46.378-4.162-66.3-6.447-23.428-2.72-37.81-3.557-58.956 2.415-18.843 5.331-39.387 6.31-58.917 7.001-20.392.712-32.074 10.527-51.29 12.6-22.8 2.454-34.474-3.927-53.467-9.487-19.321-5.657-43.09-5.63-63.489-9.344-17.823-3.224-27.253.743-45.783 3.785-22.191 3.647-34.161-.626-53.75-6.193C8.706 14.68 3.616 13.91-1 13.76V128Z"
    ></path>
  </svg>
);
const svg3 = (
  <svg
    className="absolute -bottom-1 w-full -scale-x-100"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 1440 128"
    aria-hidden="true"
  >
    <path
      fill="#f0f1d1"
      fill-rule="evenodd"
      d="M-1 128h1441V10.346c-15.931 1.765-32.565 2.85-46.786 7.017-18.994 5.56-30.708 22.76-53.462 20.307-19.23-2.074-30.91-11.889-51.297-12.6-19.53-.693-40.107-1.67-58.942-7.002-21.126-5.972-35.514-5.135-58.943-2.415-19.923 2.285-45.998 8.164-66.307 6.447-20.595-1.769-32.584-20.241-54.547-18.186-15.56 1.442-28.06 12.484-47.419 8.861-16.9-3.164-25.886-13.905-46.005-12.678-18.156 1.115-35.18 10.787-51.368 14.799-9.106 2.218-18.602 6.33-28.485 7.212-8.117.743-17.501-1.64-25.455-.301-18.515 3.093-27.66 7.68-46.94 4.012-6.875-1.304-12.86-1.899-18.517-1.899-5.67 0-11.642.595-18.53 1.9-19.26 3.667-28.404-.92-46.926-4.013-7.96-1.338-17.34 1.044-25.469.3-9.87-.88-19.387-4.993-28.478-7.211C649.943 10.884 632.9 1.212 614.775.097c-20.137-1.227-29.124 9.514-46.05 12.678-19.335 3.623-31.833-7.419-47.392-8.861-21.964-2.055-33.94 16.417-54.555 18.186-20.32 1.717-46.378-4.162-66.3-6.447-23.428-2.72-37.81-3.557-58.956 2.415-18.843 5.331-39.387 6.31-58.917 7.001-20.392.712-32.074 10.527-51.29 12.6-22.8 2.454-34.474-3.927-53.467-9.487-19.321-5.657-43.09-5.63-63.489-9.344-17.823-3.224-27.253.743-45.783 3.785-22.191 3.647-34.161-.626-53.75-6.193C8.706 14.68 3.616 13.91-1 13.76V128Z"
    ></path>
  </svg>
);
const svg4 = (
  <svg
    className="absolute -bottom-1 w-full "
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 1440 128"
    aria-hidden="true"
  >
    <path
      fill="#d3d23c"
      fill-rule="evenodd"
      d="M-1 128h1441V10.346c-15.931 1.765-32.565 2.85-46.786 7.017-18.994 5.56-30.708 22.76-53.462 20.307-19.23-2.074-30.91-11.889-51.297-12.6-19.53-.693-40.107-1.67-58.942-7.002-21.126-5.972-35.514-5.135-58.943-2.415-19.923 2.285-45.998 8.164-66.307 6.447-20.595-1.769-32.584-20.241-54.547-18.186-15.56 1.442-28.06 12.484-47.419 8.861-16.9-3.164-25.886-13.905-46.005-12.678-18.156 1.115-35.18 10.787-51.368 14.799-9.106 2.218-18.602 6.33-28.485 7.212-8.117.743-17.501-1.64-25.455-.301-18.515 3.093-27.66 7.68-46.94 4.012-6.875-1.304-12.86-1.899-18.517-1.899-5.67 0-11.642.595-18.53 1.9-19.26 3.667-28.404-.92-46.926-4.013-7.96-1.338-17.34 1.044-25.469.3-9.87-.88-19.387-4.993-28.478-7.211C649.943 10.884 632.9 1.212 614.775.097c-20.137-1.227-29.124 9.514-46.05 12.678-19.335 3.623-31.833-7.419-47.392-8.861-21.964-2.055-33.94 16.417-54.555 18.186-20.32 1.717-46.378-4.162-66.3-6.447-23.428-2.72-37.81-3.557-58.956 2.415-18.843 5.331-39.387 6.31-58.917 7.001-20.392.712-32.074 10.527-51.29 12.6-22.8 2.454-34.474-3.927-53.467-9.487-19.321-5.657-43.09-5.63-63.489-9.344-17.823-3.224-27.253.743-45.783 3.785-22.191 3.647-34.161-.626-53.75-6.193C8.706 14.68 3.616 13.91-1 13.76V128Z"
    ></path>
  </svg>
);

const Content = (): React.JSX.Element => {
  let s = "string";

  return (
    <>
      <main>
        {/* This one shows one way of applying two svgs at the bottom */}
        <div className="h-screen overflow-hidden bg-blue-300 text-4xl font-bold text-white/80">
          <div
            style={{
              backgroundImage: `url(${cloud})`,
            }}
            className="h-full w-[2880px] animate-marquee bg-bottom bg-repeat-x"
          >
            <div className="absolute left-1/2 top-[48vh] z-10 -translate-x-1/2 -translate-y-1/2 text-center  text-[#1b6d3f]">
              <h1 className="text-6xl">Hello message, bla bla!</h1>
              <h2 className="mt-10 text-4xl">Check this out!</h2>
              <button className="mt-10 rounded-md border-4 border-[#cc1d1d] bg-[#cc1d1d] px-6 py-2 text-xl text-white transition-transform hover:scale-105">
                Explore &rarr;
              </button>
            </div>
            <div className="relative flex h-full items-end justify-center ">
              {svg}
              {svg2}
            </div>
          </div>
        </div>
        <div className="relative h-screen overflow-hidden bg-[#d3d23c] text-4xl font-bold text-white">
          <div className="mt-24 flex h-[60%] flex-col items-center justify-center gap-6 lg:mt-0 lg:flex-row">
            <div className="mx-4 min-w-[300px] max-w-[500px] border-[10px] border-white/90 shadow-md lg:ml-10 lg:-rotate-3">
              <img className="" src={citrus}></img>
            </div>
            <div className="mx-4 mb-10 max-w-[500px] text-xl text-[#413528]/80 sm:mt-4 sm:text-2xl">
              <h2 className="mb-6 text-2xl  text-[#413528] sm:text-4xl ">
                Crazy title!
              </h2>
              Lorem ipsum dolor, sit amet consectetur adipisicing elit.
              Asperiores quis aspernatur atque, iure quas pariatur deserunt
              doloremque. Exercitationem eaque animi modi, perspiciatis iure
              ipsum deserunt reprehenderit esse sint cupiditate odit?
            </div>
          </div>
          <div className="h-[40%] w-[2880px] ">
            <div className="relative h-full">{svg3}</div>
          </div>
        </div>
        <div className="relative h-screen overflow-hidden bg-[#f0f1d1] text-4xl font-bold text-white">
          <div className="mt-24 flex h-[60%] flex-col items-center justify-center gap-6 lg:mt-0 lg:flex-row">
            <div className="mx-4 mb-10 max-w-[500px]  text-xl text-[#413528]/80 sm:mt-4 sm:text-2xl">
              <h2 className="mb-6 text-2xl  text-[#413528] sm:text-4xl ">
                Crazy title!
              </h2>
              Lorem ipsum dolor, sit amet consectetur adipisicing elit.
              Asperiores quis aspernatur atque, iure quas pariatur deserunt
              doloremque. Exercitationem eaque animi modi, perspiciatis iure
              ipsum deserunt reprehenderit esse sint cupiditate odit?
            </div>
            <div className="mx-4 min-w-[300px] max-w-[500px] border-[10px] border-white/90 shadow-md lg:ml-10 lg:rotate-3">
              <img className="" src={citrus}></img>
            </div>
          </div>
          <div className="h-[40%] w-[2880px] ">
            <div className="relative h-full">{svg4}</div>
          </div>
        </div>
        <div className="relative h-screen bg-[#d3d23c] text-4xl font-bold text-white"></div>
      </main>
    </>
  );
};

export default Content;
